...
// Note: Don't explicitly specify dialectModule - let Sequelize auto-detect mysql2
// This prevents crashes if mysql2 isn't available during module load
let sequelize;
let db = {};
...
try {
  sequelize = new Sequelize(process.env.MYSQL_URI, {
    dialect: "mysql",
    // Remove dialectModule - Sequelize will auto-detect mysql2 if available
  ...
  // Export the properly initialized db object
  module.exports = db;
} catch (error) {
  // Handle initialization errors gracefully
  const errorMsg = `Failed to initialize Sequelize: ${error.message}`;
  console.error(errorMsg);
  // Create a dummy sequelize instance
  sequelize = {
  ...
  db.Department = null;
  // Export early to prevent further execution
  module.exports = db;
}